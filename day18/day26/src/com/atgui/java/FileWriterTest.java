...
public class FileWriterTest {
    ...
    说明点：
    1.reade()的理解：返回读入的一个字符。如果达到文件末尾，返回-1
    2.异常的处理：为了保证刘资源一定可以执行关闭操作。需要使用try-catch-finally处理
...
1.输出操作,对应的File可以不存在的、并不会报异常
 2.  File对应的硬盘中的文件如果不存在，在输出的过程中，会自动创建文件
    File对应的硬盘中的文件如果存在：
            如果流使用的构造器是FileWriter（file，false）/FileWriter(file):对源文件的覆盖
            如果流使用的构造器是FileWriter（file，true）：不会对原有文件进行覆盖，而是在原有文件上追加内容
